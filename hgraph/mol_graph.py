...
class MolGraph(object):
    ...
    def join_separated_graphs(
        self, graph: nx.DiGraph or nx.Graph, ud_graph: nx.Graph = None
    ):
        if ud_graph is None:
            return self.join_separated_trees(graph)
        else:
            return self.join_separated_di_graphs(graph, ud_graph)

    def get_union_heuristic(self, ud_graph: nx.Graph):
        substuctures = [list(x) for x in nx.connected_components(ud_graph)]
        atom_types = [
            [self.mol.GetAtomWithIdx(x).GetSymbol() for x in conn]
            for conn in substuctures
        ]
        atom_types_set = [set(x) for x in atom_types]
        for b1, b2 in self.BOND_HEURISTICS:
            if b1 in atom_types_set[0] and b2 in atom_types_set[1]:
                match1 = substuctures[0][atom_types[0].index(b1)]
                match2 = substuctures[1][atom_types[1].index(b2)]
    ...
    def join_separated_di_graphs(self, graph: nx.DiGraph, ud_graph: nx.Graph):
        charged = self.get_salt_link_elements(ud_graph)
        lst = [None, None]
        for i, connected_component in enumerate(nx.connected_components(ud_graph)):
            for atom in charged:
                if atom in connected_component:
                    if lst[i] is not None:
                        raise ValueError("Repeated link in charged element")
                    lst[i] = atom

        graph.add_edge(*lst, weight=1)
        graph.add_edge(*reversed(lst), weight=1)
        btype = MolGraph.BOND_LIST.index("salt_link")
        graph[lst[0]][lst[1]]["label"] = btype
        graph[lst[1]][lst[0]]["label"] = btype
        return graph

    def join_separated_trees(self, graph):
        charged = self.get_salt_link_elements(graph)
    ...
    def find_charged_elements(self):
        charged_atoms = []
